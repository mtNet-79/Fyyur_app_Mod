...
from my_app.hello import models
from my_app.hello.models import *
from my_app.hello.forms import *
import datetime
...
@app.route('/venues/search/location', methods=['POST'])
def search_by_location():
  loc_id = request.form['search_location_term']
  city = City.query.get(loc_id)
  venues = Venue.query.filter_by(city_id=city.id).all()
  search_term = city.name+', '+city.state
  returns = {}
  returns['count'] = 0
  if venues:
    returns['data'] = []
    for rcrd in venues:
      ...
      dict['name']=rcrd.name 
      returns['data'].append(dict)
  return render_template('pages/search_venues.html', results=returns, search_term=search_term)
  
 


@app.route('/artists/search/location', methods=['POST'])
def search_artist_location():
  loc_id = request.form['search_location_term']
  city = City.query.get(loc_id)
  artists = Artist.query.filter_by(city_id=city.id).all()
  search_term = city.name+', '+city.state
  returns = {}
  returns['count'] = 0
  ...
  return render_template('pages/search_artists.html', results=returns, search_term=search_term)
